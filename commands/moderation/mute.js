...
module.exports = {
    ...
    async execute(interaction, client) {
        ...
        if (member.roles.highest.position >= interaction.member.roles.highest.position) {
            return interaction.reply({ embeds: [errorEmbed('Vous ne pouvez pas rendre muet un utilisateur ayant un rôle supérieur ou égal au vôtre.')], ephemeral: true });
        }
        ...
        try {
            ...
            await user.send({ embeds: [new EmbedBuilder().setColor(config.colors.danger).setDescription(`${config.emojis.mute} Vous avez été **rendu muet** sur **${interaction.guild.name}**\n**Durée :** ${formatDuration(duration)}\n**Raison :** ${reason}`)] }).catch(() => {});

            db.addSanction(interaction.guild.id, user.id, interaction.user.id, 'mute', reason, durationStr);

            await interaction.reply({ embeds: [successEmbed(`**${user.tag}** a été rendu muet pour **${formatDuration(duration)}**.\n**Raison :** ${reason}`)] });
        } catch (error) {
            await interaction.reply({ embeds: [errorEmbed('Une erreur est survenue lors du mute.')], ephemeral: true });
        }
    },
};
